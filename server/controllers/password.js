...
exports.recover = async (req, res) => {
    try{
        ...
        if(!user) return res.status(401).json({message: "El correo " + req.body.email + " no esta asociado a ninguna cuenta"});
        ...
        let html = `<p>Hola ${user.username}</p>
                    <p>Por favor ve a <a href="${link}">link</a> Para restablecer tu contraseña.</p> 
                    <p>Si no pediste esto, ignoralo, tu contraseña no será cambiada.</p>`;

        await sendEmail({to, from, subject, html});

        res.status(200).json({message: "Un email para restablecer tu contraseña fue enviado a " + user.email + "."});
    } catch (error) {
...
exports.reset = async (req, res) => {
    try {
        ...
        if (!user) return res.status(401).json({message: 'El token para restablecer tu contraseña es invalido o a expirado.'});
